...
DialogProc proc hWnd:DWORD, uMsg:DWORD, wParam:DWORD, lParam:DWORD 
   ...
   .elseif uMsg == WM_COMMAND 
		.if wParam == 301 
			...
			szText libName, "MyDll.dll"
			szText FuncName1, "MyMessage"
			szText FuncName2, "MyADD"
            let hLib, LoadLibrary,ADDR libName
            invoke GetProcAddress, hLib, ADDR FuncName1
            call eax	; виклик 1-ї функції без параметрів
			szText szResult, "Результат функції MyADD"
			szText szFormat, "X + Y = %d ",0 
			invoke GetProcAddress, hLib, ADDR FuncName2
			push X
			push Y
			call eax 	; виклик 2-ї функції з 2 параметрами
